...
export class StartupService {
    ...
    load(): Promise<any> {
        ...
        return new Promise((resolve, reject) => {
            ...
            zip(
                this.httpClient.get(SystemResource.localResource.url + `/assets/i18n/${this.i18n.defaultLang}.json`),
                this.httpClient.get(SystemResource.localResource.url + '/assets/app-data.json')
            ).pipe(
                // 接收其他拦截器后产生的异常消息
                ...
                // 判断用户是否已经登录过系统
                const currentSystem: any = this.cacheService.getNone('currentConfig');
                // 1、未进行登录
                // 读取配置信息，并进行缓存
                if (!currentSystem && user) {
                    this.cacheService.clearNotify();
                    this.tokenService.clear();
                } else if (currentSystem && currentSystem.name === 'setting') { // 用户已经选择了系统并进行过登录
                    this.menuService.add(res.menu);
                } else if (currentSystem && currentSystem.name === 'app') {
